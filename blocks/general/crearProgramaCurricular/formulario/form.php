...
namespace general\crearProgramaCurricular\formulario;
...
class Formulario {
	...
	function formulario() {
		...
		$matrizFacultades = $esteRecurso->ejecutarAcceso ( $atributos ['cadena_sql'], "busqueda" );
		
		if ($matrizFacultades) {
			$atributos ['matrizItems'] = $matrizFacultades;
		}
		...
		if ($matrizFacultades) {
			...
			echo "**PRIMERO REGISTRE UNA FACULTAD";
		}
		unset ( $atributos );
		...
		if ($matrizFacultades) {
			...
			$atributos ['tabIndex'] = $tab;
			$atributos ['etiqueta'] = $this->lenguaje->getCadena ( $esteCampo );
			$atributos ['validar'] = 'required, maxSize[100], custom[email]';
