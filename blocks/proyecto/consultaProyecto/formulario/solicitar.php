...
class Formulario {
	...
	function formulario() {
		...
		//buscar versi�n del ultimo documento
		
		$atributos ['cadena_sql'] = $this->miSql->getCadenaSql ( 'buscarVersionDoc', $documentoProyecto[0][0] );
		$docProyecto = $esteRecurso->ejecutarAcceso ( $atributos ['cadena_sql'], 'busqueda' );
		//var_dump($docProyecto);
...
<?php
		...
		?>
					...
					<tr>
						...
						<td>Versión No. <?php echo $docProyecto[0][0];?></td>
					</tr>
				</table>
							...
							<br>
								<div class="mensaje-ayuda">
									<div>IMPORTANTE: La versión del documento a radicar como
										proyecto es la última subida al sistema. Por favor verifique
										si el número corresponde a la versión que realmente desea que
										sea evaluada.</div>
								</div>
			...
			<Blockquote>Por favor diligencie todas las preguntas establecidas.
				Recuerde que las justificaciones son de caracter obligatorio.</Blockquote>
...
<?php
		...
		$valorCodificado .= "&documento=" . $documentoProyecto [0] [0];
